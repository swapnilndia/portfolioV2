    ...
    return (
      <div className="message-renderer">
        ...
        {Array.isArray(sections) && sections.length > 0 && sections.map((section, index) => {
          ...
          return (
            <div key={index} className={`message-renderer__section message-renderer__section--${section.type}`}>
...
function renderImage(data: ImageData): React.ReactNode {
  ...
  return (
    <div className="message-renderer__image-wrapper">
      ...
      />
      {data.alt && (
        <div className="message-renderer__image-caption">{String(data.alt)}</div>
      )}
    </div>
  )
}
